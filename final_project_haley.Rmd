...
media%>%
  ggplot(aes(y = yearly_articles/yearly_shootings, x= as.numeric(year), color = as.numeric(year)))+ 
  geom_point()
...
fit = glm(incident_count~gpa+target+random_vict+bullied+domestic_violence+ offset(log(population)), family = "poisson", data = ss_counts)
