...
gee_dat_grade = ss_counts%>%
  select(c(1,2,3,20,22,23,25,26,28,30,32,34,36:39))

gee_grade = geeglm(incident_count~gpa+poverty+bullying_score+ prev_year_art+year+suicide_score+fight_score+school_safety_score+bully_score,family=poisson("log")
                ,offset = log(population),id=as.factor(state),data=gee_dat_grade[complete.cases(gee_dat_grade),])
...
gee_dat_law = ss_counts%>%
  select(c(1,2,3,20,23,25,26,28,30,32,34,36:39))

gee_law = geeglm(incident_count ~ prev_year_art + poverty + suicide_score + school_safety_score + fight_score + bully_score + access_laws + 
    transport + screening + ban,family=poisson("log")
                ,offset = log(population),id=as.factor(state),data=gee_dat_law[complete.cases(gee_dat_law),])
...
ss_counts%>%
  mutate(grade = substr(grade,1,1))%>%
  group_by(grade)%>%
  summarise(count = sum(incident_count),
            pop = sum(population))%>%
  filter(!is.na(grade))%>%
