...
```{r}
rand.count.data = left_join(rand.count.data,states, by = c("state"="state.name"))%>%
  select(-c(state,`0_Permissive`,`0_Restrictive`))%>%
  rename(state = state.abb)
...
fit = glm(incident_count~gpa+poverty+bullying_score+mh_score+ prev_year_art+year+suicide_score+fight_score+school_safety_score+bully_score+ offset(log(population)), family = "poisson", data = ss_counts)
