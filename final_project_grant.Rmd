...
rand.count.data=unite(rand.count.data,"law_effect",c("type_of_law","effect"),sep="_")%>%spread("law_effect","count")%>%clean_names()%>%write.csv("cleaned.rand.csv")
}

# d=cbind(y,df)
# reformula(names(d)[-1],names(d[col numbers u want]))
