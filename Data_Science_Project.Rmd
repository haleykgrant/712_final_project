...
#### Model using laws:
```{r}
# need to add in zero for rows with no shootings
...
# fit gee
gee_dat_grade = ss_counts%>%
  select(c(1,2,4,20,22,23,25,26,28,30,32,34))

gee_grade = geeglm(total_victims~gpa+poverty+bullying_score+ prev_year_art+year+suicide_score+fight_score+school_safety_score+bully_score,family=poisson("log") ,offset = log(population),id=as.factor(state), corstr = "ar1", data=gee_dat_grade[complete.cases(gee_dat_grade),]) 


z_fit = pscl::zeroinfl(reformulate(names(ss_counts)[c(2,22,23,25,26,28,30,32,34)],names(ss_counts)[4]), offset=log(population), dist  = "poisson", data = ss_counts%>%
  mutate(total_victims = ifelse(is.na(total_victims),0,total_victims)))
