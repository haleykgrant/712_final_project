...
author: "Haley Grant, Elizabeth Sarker, and Grant Schumock"
output:
  html_document:
    highlight: tango
    number_sections: no
...
[Link to project video](https://www.youtube.com/watch?v=rwyX9dr-9FA&fbclid=IwAR3Vib0xxFGBl3JbBmlFXy8sMGwRapV5oBgnYbfGdRE6t7TlsYzVy08_Qpc)

[Link to project website](https://datasci.data.blog/2019/12/11/school-shootings-do-gun-laws-work/?fbclid=IwAR0VxPZ1eJsdJFoDK6notJQMcWZn3GBkezulPZRE35hHcJgzn4O5RX8G9Lo)

[Link to GitHub Repository](https://github.com/haleykgrant/712_final_project)

__Data are all included (in cleaned format) in the GitHub repository. We also include links to all sources of data in the [data](#data) section of this HTML file.__
...
library(viridis) 
library(tscount)
library(yrbss) #install using devtools::install_github("gsimchoni/yrbss")
library(zoo)
library(purrr)
library(gganimate)
library(usmap)
...
We determined that observations with a reliability score of 1 may not be accurate, and hence discarded these observations from further analysis. The dataset also has a variable indicating the category/cause of the shooting incidences. Any observation marked as "Accidental" was also removed from the data before any subsequent analysis, though only a small proportion of the data fell into this category.
...
## Gun Laws
This analysis also wishes to investigate the impact of specific types of gun laws separately, and thus we also included the RAND dataset. The dataset was obtained from the [RAND State Firearm Law Database](https://www.rand.org/pubs/tools/TL283-1.html). The data include information about the changes in firearm laws for each state since the year 1979. The dataset contains variables with information about the type of the law, whether the change was restrictive or permissive, effective date, and end date. There are total 14 types of laws and each change is designated as either restrictive or permissive. For our analysis, we count the number of laws in effect in each state and year. As some of the laws change in the middle of a year, the dates are rounded to the nearest year (cutoff point of July 1st). Each permissive law is assigned the value -1 and each restrictive law is assigned a +1. Then, for each category, the total number of permissive and restrictive laws are summed to obtain a final tally for each state in each year. The different types of laws are further combined into the following four categories:
...
3. Ban: firearm ban
4. Transport laws: castle doctrine, open carry, concealed carry
...
# combine law types by a simple set of phrases that all laws contain exactly one of 
...
# Only keep laws that have non-missing values for the type of change and effect, and remove DC
...
# Determine the years in which the law is in effect by rounding the start and end dates to the nearest year. For laws that are not superceded by newer laws, set the end date to 2020 (our analyses do not extend to 2020 so this is equivalent to saying the law never ended).
...
## Mental Health 
The literature review shows a strong connection between mental illness and school shooting occurrences. Thus, we collected a dataset that contains youth mental health data by state. The dataset was obtained from [Mental Health America](https://mhanational.org/issues/mental-health-america-youth-data). The dataset contains the percentage of youths who had a major depressive episode but did not receive any mental health treatment in each state over the years 2010 to 2016.  
...
## CDC Survey 
Our literature review indicates that mental health, school safety, and bullying all play a vital role in school shootings. To analyze the school shooting trend in the United States, we gathered data from CDC's survey, "Youth Risk Behaviour Surveillance System," given to students in 37 states biannually from 1991 to 2015. The data were obtained from the R package [`yrbss`](https://github.com/gsimchoni/yrbss). The dataset includes proportions of students answering "yes" to a number of questions. We selected and include in the analysis the following questions: 
...
Because many of these questions cover similar topics, we choose to combine the variables into four main categories and take weighted averages (weighted by the number of respondents) to obtain a final score for each category. The categories are classified as follows:
...
Data for questions regarding bullying were not present in the dataset prior to 2010. We assume the surveys were updated to include measures of bullying after the initial writing of the survey. The dataset also only contains data for every other year. To get data for the missing years, we impute the missing observations with the mean of the values in years immediately preceding and following the missing observations. As the dataset only contains data up to the year 2015, we extrapolate the values for the years after 2015 years by predicting a linear trajectory using the 2014 and 2015 data points. This was done after exploratory data analysis showed that proportions were relatively stable across years in a given state. To impute values for states not present in the dataset, we use the median score across all states accounted for in the data for each year (we chose to use the median after seeing that the mean and the median values in each year were nearly identical). 
...
## Poverty
The financial status of populations within each state may also have an influence on the number of school shootings that took place each year. Thus, to account for this, we include the poverty prevalence rate for each state over the study period. This was measured by the percent of the population living below the poverty line. The dataset was obtained from [The U.S. Census Bureau](https://www.census.gov/library/publications.html). The dataset contains the a 3-year average of poverty rate and a 90% confidence interval for each state over the years 2007 to 2018. We assign these 3-year averages to the corresponding three years, which assumes that poverty rate remained constant over these periods.
...
Based on the indications from the literature review, we also wish to consider the impact of media coverage on school shooting rates. A dataset was retrieved from a [Nature article](https://www.nature.com/articles/s41562-019-0636-0?fbclid=IwAR2mCmg1gQ1OOccfjklm4HKzOMcpg24Oz2C8ZOebg12HaBWwr6Up1YxosF4#data-availability), which details media coverage and firearm acquisition after mass shootings (data available on the following [GitHub repository](https://github.com/Causality-Research/Firearms)). The dataset reports the number of articles about shootings published in the Washington Post and The New York Times, as well as the number of mass shootings in the U.S. for every month, starting in January 1999 and ending in December 2017. We use the number of articles about shootings excluding those about laws and regulations as a variable in our analysis. In our analysis, we consider this variable with a time-lag, assuming that the number of articles in the year leading up to an event may have had an influence on the occurrence of that event.
...
## Population 
It is well known that states vary widely in terms of population size. Because of this--and because number of events is our outcome of interest--we must take population into account. This is because states with much higher populations are more likely to experience a higher number of events than smaller states. We obtained state population data from the [U.S. Census Bureau](https://scholar.harvard.edu/files/awatanabe/files/us_state_population_estimate_technical_notev1.pdf?fbclid=IwAR2VQnO9wWk1dlJT01_8LTqdTP4qet8wACZTIBJ9NpFbkyn_3-o1yM1bGRw). The dataset contains the population of each state from 1970 to 2019. 
...
When we look further into the mental health dataset, we realize there are many issues with these data. For one, the data for each year were obtained from a range of years, rather than a single year. Moreover, some of these ranges were identical but the values disagreed. Further, the reported percentages do not always change from year to year. The plot below shows an example of this, assuming the year prior to the report was the survey year. 
...
__Note__: population was not used as a variable, but as an offset term in the model (coefficient set to be 1, not estimated). Note we also have only one year's worth of data for the "strength of anti-bullying laws" variable so we use it as a fixed state effect (doesn't vary over time). Media data only varies by year, not by state.
...
where here for each state $s$ and year $t$ we have that $I_{t,s}$ is the school shooting incident count, $g_{t,s}$ is the grade, $p_{t,s}$ is the percent of the population living in poverty, $b_{t,s}$ is the survey-derived bullying score, $l_{s}$ is the state anti-bullying law score (does not vary by year), $s_{t,s}$ is the survey-derived suicide score, $t$ is time (year), $f_{t,s}$ is the survey-derived physical altercation score, $y_{t,s}$ is the survey derived school danger score, $m_{t-1}$ is the media score (number of articles) in the previous year, and $n_{t,s}$ is the population.


## Model Results
Below we show the model output. One can see that the only significant covariate in this model is GPA (state gun policy grade). The estimate is negative, indicating that higher GPA (stricter gun laws) is associated with fewer school shootings.
...
From this model we predict that with a one unit change in GPA, a state would expect to see a __27.3% reduction (95%  C.I. (16.5%,36.7%))__ in school shootings. This model also implies that if GPA is increased by 4 (going from an F to an A), the expected drop in school shootings is __72.1% (95%  C.I. (51.4%,84%))__. For context, the average number of school shootings per year and state in the data is 0.516.

## Model Checking

To check the model, we run a number of diagnostic tests. First, we perform a deviance test and get a p-value of 0.838, indicating no evidence against our model fit. We also test for overdispersion and get a p-value of 0.945 and estimated dispersion parameter of -0.1, giving no strong evidence of overdispersion. Lastly, we make diagnostic plots and see that the model fits relatively well.
...
Finally, we wish to see if there is variability between states that we did not already account for that could be affecting school shooting counts. To do this we apply Generalized Estimating Equations (GEE), taking state as the grouping factor. We use an autoregressive correlation structure within states to take into account the fading lag effects across years (with years closer in time assumed to be more highly correlated than distant years). We can see below that even when taking state variation into account, GPA still has a significant effect in reducing school shootings. The results are very similar to our previous GLM results.
...
Our model estimates that every addition of a restrictive (or removal of permissive) access law is associated with a __6.4% (95% C.I. (1.3%,11.3%)) drop__ in school shootings and, likewise, every addition of a ban is associated with a __22.9% decrease in school shootings (95% C.I. (2.0%,39.4%))__.
```{r}
...
## Model Checking
Again, we check our model fit using a variety of methods. For our deviance test we get no significant evidence against our model (p = 0.9593). We also do not get evidence of overdispersion (p = 0.3599) with an estimated dispersion parameter of 0.0181. The diagnostic plots show an even better fit than our first model (red lines look more horizontal than in previous model). 
...
We, again, use GEE to check for unaccounted-for variability between states. Interestingly, for this set of covariates, the model output does not match our previous output with respect to significance (direction of estimated coefficients still similar). Here none of the types of law show a significant effect on school shootings with p-values all greater than 0.4. We believe one explanation for this is because these laws do not change drastically from year to year, they may be absorbed into the state effect and hence appear insignificant after allowing for state variation. This does not necessarily mean that these laws do not play a role in school shootings. Rather, this could just be a consequence of the laws' lack of variation across time, making their effects difficult to distinguish, using the data we have, from other within-state variability. 
...
Here we see that there all four types of gun laws have evidence of association with number of victims. Below we show the estimated change in number of victims from school shootings per unit change in each type of gun law (negative values correspond to decreases in victim counts). For context, the average number of victims in a given and year is 0.856. Interestingly, in this analysis, screening and transport laws both have positive coefficients, indicating that stricter laws are associated with more fatalities/injuries from school shootings. While the effect of screening laws is relatively small, the transport laws have a sizable effect. Further analysis may want to look at these laws in further detail to understand this finding.
...
This analysis found that gun policies are, in fact, associated with school shooting incident rates. Our models show an estimated 27.3% decrease (95% C.I.: (16.5%, 36.7%)) in the number of school shootings for every one unit change in state gun policy grade (change by one on 4.0 scale). We also found an estimated 22.9% decrease in the number of school shootings (95% C.I. (2.0%, 39.4%)) per additional ban implemented and a 6.4% decrease (95% C.I. (1.3%, 11.3%)) per additional access law. Secondary analysis showed an expected decrease of 19.1% in the number of victims of school shootings (95% C.I (-34.9%, 0.5%) change) for each one point increase in state gun policy grade (GPA) and an estaimeted 36.1% decrease (95% C.I. (-54.3%, -10.7%) change) per additional gun ban.

## Recommendation
The analysis shows strong evidence that states with more prohibitive gun laws experience fewer school shootings on average than states with weaker policies. Because of these findings, we suggest that state legislators repeal or modify permissive gun laws and impose new restrictive laws, focusing primarily on gun bans and laws that restrict access to firearms. States with weak gun laws should work to improve their policies and states that already have strong gun policies should work to maintain them.


## Limitations

There are several limitations to the data and models used in this data analysis. The school shootings dataset relies on open source information such as news and police reports. These may contain inaccuracies and inconsistencies between sources. For the gun policy grade dataset, we relied upon determinations made by Giffords legal experts. This assumes the legal experts are unbiased and competent. Our analysis also assumes that the difference between grades is consistent; that is, the difference between an A and a B is the same as the difference between a C and a D, for example. The grade assignment also cannot determine an order among states with the same letter grade. 

Similarly when investigating the effects of different gun laws, our method relied on the assumption that all gun laws within a category are weighted equally. We also assumed, for each type of law, that a restrictive law had the same magnitude as a permissive law, assigning weights +1 and -1, respectively. Again, as mentioned earlier, some of the laws changed in the middle of a year and, thus, we lost information by deciding to round the value to the nearest year. We also do not take into account potential delayed, downstream effects of policy changes. That is, we assume that laws impact the school shooting rates during the entire period in which they are in effect, and only in that time. We also did not take into account the school-going population within each state. Rather, we used the entire state population, assuming that the proportion of the population attending school is roughly the same across all states.

Our use of the CDC survey data also has limitations. As we mentioned earlier, we had to impute many missing values in the data, both for years not recorded in the data and for states in which the survey was not conducted. Thus, we forced potentially unmet assumptions on the data in an effort to maximize usable data for our analysis.

Bullying at schools is often considered one of the crucial components of school shooting incidents. Thus, we wished to account for the variation in the existing anti-bullying laws in different states. However, the most credible dataset that was available for each state contained data only for the year 2010. We chose to include the variable to adjust for the variation in state laws but had to assume that the states maintained the same level of rigidity towards bullying throughout the study period. The analysis would have been more thorough if we had data for all years.   

The main limitation of the models fit in this analysis is that they can only show evidence of association, not causation. This means that although states with stricter gun laws appear to have lower incidence rates of school shootings (after controlling for variables such as population, poverty rate, etc.), states with weak gun laws may not actually see a decrease in their numbers of school shootings if they impose stricter gun laws. We would need to incorporate additional data sources to test whether there is a causal link between gun policies and school shooting incidence rates.


## Future Work
In future analyses, one may want to consider state unemployment rate as another factor influencing school shooting incidence rates. Media coverage also seems to play an important role in school shooting rates in this analysis. It would be interesting to look more closely at these and other data regarding media coverage of shooting events to see if it does in fact lead to more shootings. As mentioned above, other models and data that more readily lend themselves towards making causal claims regarding gun policies could be used to make more insightful recommendations to state legislators.
